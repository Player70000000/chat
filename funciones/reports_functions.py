...
def eliminar_reporte_obreros(reporte_id):
    """
    Eliminar reporte de obreros por ID
    Elimina tanto el archivo PDF como el registro de la base de datos
    """
    try:
        from bson import ObjectId
        ...
        # Obtener información del reporte para logging
        numero_reporte = reporte.get("numero_reporte", "N/A")
        total_obreros = reporte.get("total_obreros", "N/A")
        pdf_filename = reporte.get("pdf_filename", "")

        logger.info(f"🗑️ Iniciando eliminación del reporte de obreros N°{numero_reporte} - {total_obreros} obreros")
        ...
        logger.info(f"✅ Reporte de obreros eliminado exitosamente: N°{numero_reporte} - {total_obreros} obreros")

        return jsonify({
            "success": True,
            "message": f"Reporte de obreros N°{numero_reporte} eliminado exitosamente",
            "reporte_eliminado": {
                "id": str(object_id),
                "numero_reporte": numero_reporte,
                "total_obreros": total_obreros
        ...
        logger.error(f"❌ Error eliminando reporte de obreros: {str(e)}")
        return jsonify({
            "success": False,
            "error": f"Error interno del servidor: {str(e)}"
        }), 500

def eliminar_reporte_moderadores(reporte_id):
    """
    Eliminar reporte de moderadores por ID
    Elimina tanto el archivo PDF como el registro de la base de datos
    """
    try:
        from bson import ObjectId
        ...
        # Obtener información del reporte para logging
        numero_reporte = reporte.get("numero_reporte", "N/A")
        total_moderadores = reporte.get("total_moderadores", "N/A")
        pdf_filename = reporte.get("pdf_filename", "")

        logger.info(f"🗑️ Iniciando eliminación del reporte de moderadores N°{numero_reporte} - {total_moderadores} moderadores")
        ...
        logger.info(f"✅ Reporte de moderadores eliminado exitosamente: N°{numero_reporte} - {total_moderadores} moderadores")

        return jsonify({
            "success": True,
            "message": f"Reporte de moderadores N°{numero_reporte} eliminado exitosamente",
            "reporte_eliminado": {
                "id": str(object_id),
                "numero_reporte": numero_reporte,
                "total_moderadores": total_moderadores
        ...
        logger.error(f"❌ Error eliminando reporte de moderadores: {str(e)}")
        return jsonify({
            "success": False,
            "error": f"Error interno del servidor: {str(e)}"
        }), 500
