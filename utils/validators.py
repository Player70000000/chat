...
def validate_cuadrilla_data(datos):
    ...
    numero_cuadrilla = datos.get('numero_cuadrilla', '').strip()
    ...
    if not actividad:
        return None, "La actividad de la cuadrilla es obligatoria"
    
    if len(numero_cuadrilla) > 20:
        return None, "El número de cuadrilla no puede exceder 20 caracteres"
    
    if len(actividad) > 200:
        return None, "La actividad no puede exceder 200 caracteres"
    
    if not re.match(r'^[a-zA-Z0-9\s\-_]+$', numero_cuadrilla):
        return None, "El número de cuadrilla contiene caracteres no permitidos"
...
def validate_obrero_data(datos):
    ...
    if not re.match(r'^[a-zA-ZÀ-ÿ\s]+$', apellido):
        return None, "El apellido contiene caracteres no permitidos"
    
    if not re.match(r'^[0-9\-]+$', cedula):
        return None, "La cédula debe contener solo números y guiones"
